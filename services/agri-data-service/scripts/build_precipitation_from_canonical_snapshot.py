"""Build the dedicated precipitation lane from one pinned canonical signal snapshot."""
...
from agri_data_service.warehouse.schemas.climate_field_precipitation import (  # noqa: E402
    CLIMATE_FIELD_PRECIPITATION_SCHEMA,
    CLIMATE_FIELD_PRECIPITATION_STREAM,
)

CONTRACT_VERSION: Final = "climate-field-precipitation.snapshot-breakdown.v1"
...
SOURCE_PART_PREFIX: Final = f"{SOURCE_ROOT}/source=nasa-power-daily/product=PRECTOTCORR/support=surface/"

DESTINATION_METADATA_ROOT: Final = (
    f"layer={CLIMATE_FIELD_PRECIPITATION_STREAM}/_breakdown/snapshot={SOURCE_SNAPSHOT_ID}"
)
...
SOURCE_PARAMETER: Final = "PRECTOTCORR"
SIGNAL_NAME: Final = "precipitation"
SUPPORT_KEY: Final = "surface"
NORMALIZED_UNIT: Final = "mm/day"
...
class BreakdownError(RuntimeError):
    """Raised when the pinned snapshot cannot produce an exact precipitation lane."""
...
def _serialize(table: pa.Table) -> bytes:
    schema = CLIMATE_FIELD_PRECIPITATION_SCHEMA.arrow_schema
    conformed = table.select(schema.names).cast(schema).replace_schema_metadata(schema.metadata)
    conformed = conformed.sort_by([(column, "ascending") for column in CLIMATE_FIELD_PRECIPITATION_SCHEMA.sort_columns])
    buffer = io.BytesIO()
    pq.write_table(
        conformed,
        buffer,
        compression=CLIMATE_FIELD_PRECIPITATION_SCHEMA.compression,
...
def _source_part_for_ledger(ledger: Mapping[str, Any], *, year: int, month: int) -> Mapping[str, Any]:
    expected = f"source=nasa-power-daily/product=PRECTOTCORR/support=surface/year={year:04d}/month={month:02d}/"
    parts = ledger.get("parts")
    if not isinstance(parts, list):
    ...
    if len(selected) != 1:
        raise BreakdownError(
            f"source ledger contains {len(selected)} precipitation parts under {expected!r}, expected one"
        )
    part = selected[0]
    ...
    if key != expected_key or not key.startswith(SOURCE_PART_PREFIX):
        raise BreakdownError(f"source ledger precipitation part escapes the pinned prefix: {key!r}")
    return part
...
def _classify_raw_row(row: Mapping[str, Any], *, month: str) -> str | None:
    ...
    if drift:
        raise BreakdownError(f"raw precipitation partition row {row.get('id')} in {month} violates its path: {drift}")
    ...
    if (
        ...
        raise BreakdownError(f"raw precipitation row {row.get('id')} has inconsistent observation day")
    row_hash = row.get("canonical_row_sha256")
    if not isinstance(row_hash, str) or len(row_hash) != 64:
        raise BreakdownError(f"raw precipitation row {row.get('id')} has no canonical SHA-256")
    if row.get("signal_name") != SIGNAL_NAME:
        return "signal_name_not_precipitation"
    if row.get("source_parameter") != SOURCE_PARAMETER:
        return "source_parameter_not_prectotcorr"
    if row.get("normalized_unit") is None:
        return "normalized_unit_null"
    if row.get("normalized_unit") != NORMALIZED_UNIT:
        return "normalized_unit_not_mm_per_day"
    ...
    if isinstance(value, bool) or not isinstance(value, int | float):
        raise BreakdownError(f"raw precipitation row {row.get('id')} has a non-numeric normalized value")
    return None
    ...
    *,
    ...
    for group_rows in groups.values():
        ...
        if not isinstance(observed_day, date):
            raise BreakdownError("selected precipitation row has no date-valued observation_day")
    ...
    for observed_day, day_rows in by_day.items():
        if len(day_rows) != EXPECTED_CELLS_PER_DAY:
            raise BreakdownError(
                f"deduplicated precipitation {observed_day} has {len(day_rows)} cells, "
        ...
        if table.num_rows != EXPECTED_CELLS_PER_DAY:
            raise BreakdownError(f"Arrow conversion changed precipitation row count on {observed_day}")
    ...
    *,
    ...
    for summary in ledger_summaries:
        ...
        if len(payload) != int(part["byte_count"]) or _sha256(payload) != part["sha256"]:
            raise BreakdownError(f"source precipitation part {key!r} failed byte reconciliation")
        ...
        if not RAW_REQUIRED_COLUMNS.issubset(table.column_names):
            ...
            raise BreakdownError(f"source precipitation part {key!r} omits {missing}")
        rows = table.to_pylist()
        if len(rows) != int(part["row_count"]) or _row_set_digest(rows) != part["row_digest"]:
            raise BreakdownError(f"source precipitation part {key!r} failed row-digest reconciliation")
        part_sha256 = str(part["sha256"])
        for row_ordinal, row in enumerate(rows):
            exclusion = _classify_raw_row(row, month=month)
            row_id = int(row["id"])
            if row_id in seen_ids:
                raise BreakdownError(f"physical precipitation row id {row_id} repeats within {month}")
            seen_ids.add(row_id)
...
def _write_day(
    ...
    *,
    ...
    if base.num_rows != EXPECTED_CELLS_PER_DAY:
        raise BreakdownError(f"base precipitation day {observed_day} has {base.num_rows} rows")
    for row in base.to_pylist():
        ...
        if any(values is None or len(values) != count or any(value is None for value in values) for values in aligned):
            raise BreakdownError(f"base precipitation lineage is not aligned for {observed_day}/{row['cell_id']}")
        if (
            ...
            raise BreakdownError(
                f"base precipitation winner is not lineage element zero for {observed_day}/{row['cell_id']}"
            )
    ...
    objects.append(
        _write_payload(
            store,
            key=partition_path(CLIMATE_FIELD_PRECIPITATION_STREAM, "observed", 13, observed_day),
    ...
    for zoom in (9, 5, 0):
        ...
        if derived.height <= 0:
            raise BreakdownError(f"precipitation {observed_day} z{zoom} derived to no rows")
        derived_table = derived.to_arrow()
        payload = _serialize(derived_table)
        objects.append(
            _write_payload(
                store,
                key=partition_path(CLIMATE_FIELD_PRECIPITATION_STREAM, "observed", zoom, observed_day),
                payload=payload,
        ...
        objects.append(
            _write_payload(
                store,
                key=completion_marker_path(CLIMATE_FIELD_PRECIPITATION_STREAM, "observed", zoom, observed_day),
                payload=marker,
                row_count=derived.height,
                kind="completion",
                zoom=zoom,
    ...
    objects.append(
        _write_payload(
            store,
            key=completion_marker_path(CLIMATE_FIELD_PRECIPITATION_STREAM, "observed", 13, observed_day),
            payload=base_marker,
            row_count=base.num_rows,
            kind="completion",
            zoom=13,
...
def _verify_checkpoint(store: ImmutableS3, checkpoint: Mapping[str, Any], *, month: str) -> None:
    expected = {
        ...
        "source_snapshot_id": SOURCE_SNAPSHOT_ID,
        "source_manifest_sha256": SOURCE_MANIFEST_SHA256,
        "lane": CLIMATE_FIELD_PRECIPITATION_STREAM,
        "month": month,
        "precedence_contract": PRECEDENCE_CONTRACT,
    }
...
def _audit_source_chain(
    ...
    *,
    ...
    for month, summaries in month_ledgers.items():
        ...
        for summary in summaries:
            ...
            if part_key in audited_parts:
                raise BreakdownError(f"source precipitation part {part_key!r} is repeated across ledgers")
            ...
            if len(payload) != int(part["byte_count"]) or payload_sha256 != part["sha256"]:
                raise BreakdownError(f"source precipitation part {part_key!r} failed ledger byte reconciliation")
            table = _load_parquet(payload, key=part_key)
            if not RAW_REQUIRED_COLUMNS.issubset(table.column_names):
                missing = sorted(RAW_REQUIRED_COLUMNS.difference(table.column_names))
                raise BreakdownError(f"source precipitation part {part_key!r} omits {missing}")
            rows = table.to_pylist()
            if len(rows) != int(part["row_count"]) or _row_set_digest(rows) != part["row_digest"]:
                raise BreakdownError(f"source precipitation part {part_key!r} failed ledger row reconciliation")
            for row in rows:
                exclusion = _classify_raw_row(row, month=month)
                if exclusion is None:
                    eligible_rows += 1
            ...
            ledger_receipts.append(
                {
                    ...
                    "precipitation_part_key": part_key,
                    "precipitation_part_sha256": payload_sha256,
                    "precipitation_part_row_count": len(rows),
                    "precipitation_part_row_digest": str(part["row_digest"]),
                }
            )
    ...
    return {
        "contract_version": "climate-field-precipitation.source-chain-audit.v1",
        "source_snapshot_id": SOURCE_SNAPSHOT_ID,
        "source_manifest_key": SOURCE_MANIFEST_KEY,
        "source_manifest_sha256": SOURCE_MANIFEST_SHA256,
        "lane": CLIMATE_FIELD_PRECIPITATION_STREAM,
...
def _finalize(
    ...
    *,
    ...
    if source_part_keys != listed_source_keys:
        ...
        raise BreakdownError(
            f"source precipitation inventory reconciliation failed: missing={missing[:5]}, unexpected={unexpected[:5]}"
        )
    expected = {
    ...
    if drift:
        raise BreakdownError(f"precipitation snapshot-to-lane reconciliation drifted: {drift}")
    if physical_rows != eligible_rows + excluded_rows:
        raise BreakdownError("precipitation physical population does not equal eligible plus excluded rows")
    if len(set(day_names)) != EXPECTED_DAYS:
        raise BreakdownError("precipitation checkpoint set contains duplicate days")
    ...
    expected_precompletion_inventory = {
    ...
    actual_inventory = {key for key, _ in store.list_keys(f"layer={CLIMATE_FIELD_PRECIPITATION_STREAM}/")}
    if actual_inventory != expected_precompletion_inventory:
        missing = sorted(expected_precompletion_inventory - actual_inventory)
        unexpected = sorted(actual_inventory - expected_precompletion_inventory)
    ...
    expected_inventory = {*expected_precompletion_inventory, DESTINATION_COMPLETE_KEY}
    actual_inventory = {key for key, _ in store.list_keys(f"layer={CLIMATE_FIELD_PRECIPITATION_STREAM}/")}
    if actual_inventory != expected_inventory:
        missing = sorted(expected_inventory - actual_inventory)
        unexpected = sorted(actual_inventory - expected_inventory)
        raise BreakdownError(
            f"completed destination inventory is not exact: missing={missing[:5]}, unexpected={unexpected[:5]}"
        )
    return {
        "status": "complete",
        "lane": CLIMATE_FIELD_PRECIPITATION_STREAM,
