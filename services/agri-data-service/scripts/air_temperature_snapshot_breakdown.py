"""Census and break down the pinned canonical Air Temperature snapshot."""
...
OUTPUT_CONTRACT_VERSION: Final = "plantgeo.air-temperature.snapshot-product.v1"
...
class ProductContract:
    """One non-overlapping physical Air Temperature product contract."""
...
def _arguments() -> argparse.Namespace:
    ...
    build_parser = subcommands.add_parser("build", help="write all three immutable snapshot products")
...
def _classification(dimensions: Mapping[str, object]) -> tuple[ProductContract | None, str]:
    ...
    if by_parameter is None and by_signal is None:
        return None, "outside-air-temperature-scope"
    if by_parameter is None:
        return by_signal, "air-signal-with-ungoverned-parameter"
...
def census(store: ObjectStore, *, include_lineage: bool = False) -> dict[str, object]:
    """Return the exact Air Temperature population recorded by the pinned ledgers."""
...
def _family_reconciliation(report: Mapping[str, object]) -> dict[str, object]:
    ...
    air_rows = sum(product_rows.values())
    snapshot_rows = int(snapshot["manifest_rows"])
    outside_rows = snapshot_rows - air_rows
    return {
        "snapshot_rows": snapshot_rows,
        "air_temperature_physical_rows": air_rows,
        "product_physical_rows": product_rows,
        "outside_air_temperature_rows": outside_rows,
        "snapshot_equals_products_plus_outside": snapshot_rows == air_rows + outside_rows,
    }
    ...
    *,
    ...
    for month, month_lineage in sorted(grouped.items()):
        ...
        print(
            json.dumps(
                {
                    "event": "air-temperature-product-month",
                    "product_id": product.product_id,
                    "month": month,
...
def build(store: ObjectStore, *, concurrency: int, verification_mode: str) -> dict[str, object]:
    ...
    for product in PRODUCTS:
        ...
        if overlap:
            raise SnapshotContractError(f"Air Temperature products overlap on source parts: {sorted(overlap)[:3]}")
