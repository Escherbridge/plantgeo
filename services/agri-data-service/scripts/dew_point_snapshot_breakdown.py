"""Census and break down the pinned canonical dew-point snapshot."""
...
from agri_data_service.pipeline.parquet.objectstore import ObjectStore, conform_to_stream_schema
...
MAX_OUTPUT_OBJECT_COUNT: Final = 1_000
MAX_CONCURRENCY: Final = 8
VERIFICATION_MODES: Final = ("sparse", "full")
OUTPUT_CONTRACT_VERSION: Final = "plantgeo.dew-point.snapshot-product.v1"
...
class ProductContract:
    """The physical dew-point product contract."""

    product_id: str
    stream: str
    source_parameter: str
    signal_name: str
    data_source_key: str = "nasa-power-daily"
    support_key: str = "surface"
    normalized_unit: str = "C"
    original_unit: str = "C"
    cell_grid_name: str = "nasa-power-0.5-degree"
...
def _arguments() -> argparse.Namespace:
    ...
    build_parser = subcommands.add_parser("build", help="write the immutable dew-point snapshot product")
...
def _classification(dimensions: Mapping[str, object]) -> tuple[ProductContract | None, str]:
    ...
    if by_parameter is None and by_signal is None:
        return None, "outside-dew-point-scope"
    if by_parameter is None:
        return by_signal, "dew-point-signal-with-ungoverned-parameter"
...
def census(store: ObjectStore, *, include_lineage: bool = False) -> dict[str, object]:
    """Return the exact dew-point population recorded by the pinned ledgers."""
...
def _physical_output_key(product: ProductContract, month: str, source_key: str) -> str:
    ...
    return f"{_product_root(product)}kind=physical/year={year}/month={month_number}/{filename}"
...
def _family_reconciliation(report: Mapping[str, object]) -> dict[str, object]:
    ...
    dew_point_rows = sum(product_rows.values())
    snapshot_rows = int(snapshot["manifest_rows"])
    outside_rows = snapshot_rows - dew_point_rows
    return {
        "snapshot_rows": snapshot_rows,
        "dew_point_physical_rows": dew_point_rows,
        "product_physical_rows": product_rows,
        "outside_dew_point_rows": outside_rows,
        "snapshot_equals_products_plus_outside": snapshot_rows == dew_point_rows + outside_rows,
    }
...
def _verify_product(
    ...
    *,
    ...
    for checkpoint_index, receipt in enumerate(checkpoint_receipts):
        ...
        checkpoint_key = receipt.get("key")
        if not isinstance(checkpoint_key, str):
            raise SnapshotContractError(f"{manifest_key} has a checkpoint receipt without a key")
        expected_keys.add(checkpoint_key)
        ...
        for output_index, output in enumerate(physical_outputs):
            ...
            if not isinstance(output_key, str):
                raise SnapshotContractError(f"checkpoint {receipt['key']} has a physical output without a key")
        ...
        for output in rungs.values():
            ...
            output_key = output.get("key")
            if not isinstance(output_key, str):
                raise SnapshotContractError(f"checkpoint {receipt['key']} has a rung output without a key")
            expected_keys.add(output_key)
    ...
    for listed in store._backend.list_objects(store.key_for(root)):
        actual_keys.add(store.relative_key(listed.key))
        if len(actual_keys) > MAX_OUTPUT_OBJECT_COUNT:
            raise SnapshotContractError(
                f"{product.product_id} output inventory exceeds bounded limit {MAX_OUTPUT_OBJECT_COUNT}"
            )
    if actual_keys != expected_keys:
        raise SnapshotContractError(
            f"{product.product_id} output inventory mismatch: "
            f"missing={sorted(expected_keys - actual_keys)[:5]}, unexpected={sorted(actual_keys - expected_keys)[:5]}"
        )
    return {
        "status": "clean",
        "product_id": product.product_id,
...
def build(store: ObjectStore, *, concurrency: int, verification_mode: str) -> dict[str, object]:
    ...
    for product in PRODUCTS:
        ...
        if overlap:
            raise SnapshotContractError(f"dew-point products overlap on source parts: {sorted(overlap)[:3]}")
