"""The vegetation lane's serving read and source reconciliation.

Two sections, one per artifact this file is the sole test coverage for:
`planes/vegetation.py` (DuckDB/Polars serving read) and `pipeline/validation/vegetation.py` (source
reconciliation). The forecast-row provenance section left on 2026-09-18 with the forecaster itself;
it is `services/plantgeo-ml-service/tests/test_vegetation_forecast.py` now.
