...
FORECAST_TEST_DATABASE_URL = os.getenv("FORECAST_TEST_DATABASE_URL")
...
@pytest.mark.skipif(
    not FORECAST_TEST_DATABASE_URL,
    reason="set FORECAST_TEST_DATABASE_URL to a migrated disposable PlantGeo database",
)
def test_forecast_readiness_sql_reports_missing_role_gate_without_error() -> None:
    assert FORECAST_TEST_DATABASE_URL is not None
    connection = psycopg2.connect(FORECAST_TEST_DATABASE_URL)
    try:
        with connection.cursor() as cursor:
            cursor.execute("SELECT current_database()")
            database_name = cursor.fetchone()[0]
            assert database_name.startswith("plantgeo_forecast_test")
...
def test_postgresql_forecasting_functions_in_rolled_back_fixture() -> None:  # noqa: PLR0915
    assert FORECAST_TEST_DATABASE_URL is not None
    connection = psycopg2.connect(FORECAST_TEST_DATABASE_URL)
    ...
    try:
        with connection.cursor() as cursor:
            ...
            database_name = cursor.fetchone()[0]
            assert database_name.startswith("plantgeo_forecast_test"), (
                "forecast SQL integration tests refuse non-disposable databases"
            )
            ...
            assert cursor.fetchone()[0] == "hindcast_v2"
            ...
            cursor.execute(
                "SELECT DISTINCT receipt_digest_version FROM agri.v_forecast_hindcast_outcome "
                "WHERE hindcast_run_id = %s",
                (hindcast_run_id,),
            )
            assert cursor.fetchone()[0] == "hindcast_v2"
