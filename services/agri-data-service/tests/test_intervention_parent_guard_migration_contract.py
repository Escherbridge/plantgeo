...
_CANONICAL_FUNCTION = _SERVICE_ROOT / "db" / "agri" / "functions" / "protect_intervention_evidence_parents.sql"
...
def test_upgrade_forward_loads_the_canonical_function_definition() -> None:
    ...
    assert 'load_object_sql("functions/protect_intervention_evidence_parents.sql", or_replace=True)' in migration
...
def test_upgrade_uses_an_isolated_locked_function_owner() -> None:
    ...
    assert (
        f"ALTER FUNCTION agri.protect_intervention_evidence_parents()\n            OWNER TO {_GUARD_OWNER};"
    ) in migration
    assert (f"GRANT UPDATE (id) ON TABLE agri.release_set\n            TO {_GUARD_OWNER};") in migration
...
def test_direct_execution_and_schema_creation_remain_revoked() -> None:
    ...
    alter_owner = f"ALTER FUNCTION agri.protect_intervention_evidence_parents()\n            OWNER TO {_GUARD_OWNER};"
    ...
    assert migration.index(grant_create) < migration.index(alter_owner) < migration.index(revoke_owner_create)
