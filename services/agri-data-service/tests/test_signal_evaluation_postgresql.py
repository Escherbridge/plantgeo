...
DATABASE_URL = os.getenv("SIGNAL_EVAL_TEST_DATABASE_URL")
DISPOSABLE_PREFIX = "plantgeo_signal_eval_test"
...
@pytest.mark.skipif(
    not DATABASE_URL,
    reason="set SIGNAL_EVAL_TEST_DATABASE_URL to a migrated disposable PlantGeo database",
)
def test_forecast_iteration_evaluation_arithmetic_leakage_and_column_contract() -> None:  # noqa: PLR0915
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                "to_regprocedure('agri.forecast_iteration_evaluation(uuid,timestamptz)')::text"
            )
            database_name, procedure_name = cursor.fetchone()
...
def test_drought_class_daily_series_resolution_imputation_and_leakage_gates() -> None:  # noqa: PLR0915
    assert DATABASE_URL is not None
    connection = psycopg2.connect(DATABASE_URL)
    connection.autocommit = False
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT current_database(), to_regprocedure("
                "'agri.drought_class_daily_series(uuid,timestamptz,timestamptz,timestamptz)')::text"
            )
            database_name, procedure_name = cursor.fetchone()
            assert database_name.startswith(DISPOSABLE_PREFIX)
            assert procedure_name is not None
