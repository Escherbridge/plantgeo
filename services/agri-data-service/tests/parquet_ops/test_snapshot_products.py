...
def test_the_six_lanes_that_moved_to_the_census_are_not_reachable_as_snapshot_products() -> None:
    ...
    for layer in (
        ...
        assert snapshot_products.serves_from_snapshot(layer, CLIMATE_DIRECT_WRITER_START_DAY) is False
        with pytest.raises(ServingRefusalError) as raised:
...
#: the lane's REGISTERED schema for spatial support and declared list cells. Registration is the ONLY
#: property borrowed here: this stream left `SNAPSHOT_PRODUCTS` on 2026-09-07, and every test below
#: monkeypatches the allowlist with a product it builds itself, so allowlist membership is irrelevant.
...
def _forward_product(layer: str = FORWARD_LAYER) -> SnapshotProduct:
    """A daily product frozen only BELOW `forward_first_day`, which is the four climate products' shape."""
