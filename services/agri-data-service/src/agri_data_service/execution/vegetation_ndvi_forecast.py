"""Deterministic seasonal-anomaly Monte Carlo for sparse daily NDVI cell series; see execution/AGENTS.md."""
...
def build_seasonal_history(observations: tuple[ObservedDay, ...], cutoff_day: date) -> SeasonalHistory:
    ...
    # An observation whose own seasonal window has too few neighbours has no honest seasonal
    # reference, so it is dropped from the anomaly pool rather than referenced to a fabricated level.
