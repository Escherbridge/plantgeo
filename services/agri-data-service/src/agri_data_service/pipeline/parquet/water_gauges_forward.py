...
publisher day was `written`, so one per-day refusal failed the whole fetch -- the construct that, on
`sensors-direct-forward`, turned two stale absence markers into three consecutive exit-1 buckets and a
week-long breaker hold over days that were never broken (`pipeline/direct/sensors/forward.py`). On this
lane the hazard is live and ordinary rather than historical: one IV snapshot names a publisher day per
gauge `updatedAt`, so a single stale gauge can drag an old day into the bucket, and that day's
`merge_water_gauges_day` "ambiguous refresh" refusal (`pipeline/direct/AGENTS.md`, "Water gauges") would
have failed every other day's clean write with it. `_bucket_verdict` separates the two claims the exit
code was conflating: `outcome` stays `complete`/`incomplete` -- the word
`pipeline/direct/__init__.py::INCOMPLETE` reserves for a monitor,
...
so a partial bucket at exit 0 is visible in the ledger; the stderr `water_gauges_forward_bucket_incomplete` event is
kept
because stderr is teed to the log stream on every exit -- while the exit code says whether the lane can write AT ALL:
0 when at
least one day published, 1 only when none did. A partial bucket is therefore `incomplete` at exit 0, with
every unwritten day listed by outcome and detail under `unwritten`, and because exit 0 would otherwise
leave the degradation invisible, `_report_bucket_incomplete` ALSO emits
`water_gauges_forward_bucket_incomplete` on stderr with the same list. A partial bucket thus neither
passes for a clean success nor spends the lane's run on a degradation the next quarter-hourly fetch will
re-attempt. THAT RE-ATTEMPT IS WHAT MAKES EXIT 0 SOUND: every fetch reads the whole IV snapshot and
re-derives the same publisher days from it, so an unwritten day is re-selected automatically for as long
as NWIS still serves it. The rule must not be lifted into a lane whose unwritten day is NOT re-offered by
its next turn. The breaker exists for a lane that cannot write; a lane writing some of its days is
degraded, visibly, not broken.
"""
...
from agri_data_service.pipeline.direct import COMPLETE, INCOMPLETE
...
if TYPE_CHECKING:
    ...
    from agri_data_service.pipeline.direct.water_gauges import OverturnedAbsence
...
class ForwardDayResult:
    ...
    written_bytes: int
    detail: str | None
    #: The governed absence this day's fetch overturned, when it did; carried on failures too, since the
    #: marker is already gone by the time a later write can fail (`pipeline/direct/water_gauges.py`).
    absence_overturned: OverturnedAbsence | None = None


@dataclass(frozen=True, slots=True)
class ForwardBucketVerdict:
    """What one fetch's day publications add up to, and the exit status that sum earns."""
    ...
    def days_unwritten(self) -> int:
        """How many publisher days this fetch owned but did not publish, whatever the reason."""
...
def _report_bucket_incomplete(run_id: str, verdict: ForwardBucketVerdict) -> None:
    ...
    print(
        json.dumps(
            {
                "event": "water_gauges_forward_bucket_incomplete",
