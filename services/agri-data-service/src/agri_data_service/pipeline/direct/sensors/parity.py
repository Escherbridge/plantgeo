"""D2's counted parity receipt: Parquet's sensors coverage against PostgreSQL's.
...
it recovers out of NWS's live ~6-day retention before Postgres last ran (`source.py`, "rolling
floor") -- and that is not under-coverage. D2 only requires Parquet to cover what Postgres ALREADY
holds, so the walk is bounded by the days Postgres counts: one `read_completion_marker` GET per
Postgres day, never a `list_partition_keys()` over the whole stream. Matches
`weather_observations/parity.py`'s identical reasoning and the A4 tripwire it names
(`pipeline/parquet/AGENTS.md`).

THE ROLLING WINDOW MAKES "PARITY" A STATEMENT ABOUT WHAT POSTGRES STILL HOLDS, NOT ABOUT WHAT NWS
EVER PUBLISHED. NWS purges a station's observations after roughly six days
(`ingest/sensors.py::NWS_OBSERVATION_RETENTION`); the Postgres side of this comparison is
append-only and has been accreting since `LANE_REGISTRY['sensors'].history_floor` (2026-07-29,
`pipeline/parquet/lane_registry.py`), so it already holds days this forward writer could never
re-fetch from the live source today. `verdict` therefore answers exactly one question -- "does
Parquet cover every day Postgres's OWN accreted history already has" -- and says nothing about
whether a day Postgres never captured (because the old ingest cron was down, or ran less often than
NWS's retention) can still be recovered; once such a day is more than six days stale relative to
`now`, it is gone from BOTH sources' reach, permanently.
"""
...
class SensorsParityError(RuntimeError):
...
async def postgres_day_counts(session: AsyncSession, *, layer_id: str) -> dict[date, int]:
    """Return every day PostgreSQL's `geo.features` holds an exportable sensors row for."""
    ...
    for row in result.mappings():
        ...
        if day is None:
            raise SensorsParityError("Postgres day-count query returned a NULL observed_day")
...
def parquet_day_coverage(store: ObjectStore, day: date) -> tuple[str, int]:
    ...
    marker = store.read_completion_marker(SENSORS_STREAM, SENSORS_DIRECT_KIND, LANE_BASE_ZOOM_TIER, day)
    if marker is not None:
        return "data", marker.row_count
    if store.absence_exists(SENSORS_STREAM, SENSORS_DIRECT_KIND, LANE_BASE_ZOOM_TIER, day):
        return "absent", 0
    return "missing", 0
...
async def build_parity_receipt(session: AsyncSession, store: ObjectStore) -> ParityReceipt:
    ...
    layer_id = await resolve_layer_id(session, SENSORS_STREAM)
...
async def main() -> int:
    ...
    print(json.dumps({"event": "sensors_parity", **receipt.to_summary()}, sort_keys=True))
