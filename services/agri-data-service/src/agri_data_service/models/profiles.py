...
class SoilProfile(Base, UUIDMixin):
    ...
    location_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("agri.locations.id"), nullable=False, index=True
    )
...
class ClimateProfile(Base, UUIDMixin):
    ...
    source: Mapped[ClimateSource] = mapped_column(
        Enum(
            ClimateSource,
            name="climate_source",
            native_enum=False,
    ...
    monthly_precip_json: Mapped[dict[str, object] | None] = mapped_column(JSONB, nullable=True)
    monthly_temp_json: Mapped[dict[str, object] | None] = mapped_column(JSONB, nullable=True)
    fetched_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class WaterProfile(Base, UUIDMixin):
    ...
    water_table_seasonal_json: Mapped[dict[str, object] | None] = mapped_column(JSONB, nullable=True)
    fetched_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class LandUseSnapshot(Base, UUIDMixin):
    ...
    source: Mapped[LandUseSource] = mapped_column(
        Enum(
            LandUseSource,
            name="land_use_source",
            native_enum=False,
            create_constraint=True,
            values_callable=lambda members: [member.value for member in members],
        ),
    ...
    crop_history_json: Mapped[dict[str, object] | None] = mapped_column(JSONB, nullable=True)
    fetched_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
