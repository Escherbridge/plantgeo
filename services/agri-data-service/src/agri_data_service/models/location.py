...
class Location(Base, UUIDMixin, TimestampMixin):
    ...
    geometry = mapped_column(Geometry("POINT", srid=4326, spatial_index=False), nullable=False)
    bounding_box = mapped_column(Geometry("POLYGON", srid=4326, spatial_index=False), nullable=True)
    ...
    climate_profiles: Mapped[list["ClimateProfile"]] = relationship(
        back_populates="location", cascade="all, delete-orphan"
    )
    topography_profiles: Mapped[list["TopographyProfile"]] = relationship(
        back_populates="location", cascade="all, delete-orphan"
    )
    water_profiles: Mapped[list["WaterProfile"]] = relationship(back_populates="location", cascade="all, delete-orphan")
    land_use_snapshots: Mapped[list["LandUseSnapshot"]] = relationship(
        back_populates="location", cascade="all, delete-orphan"
    )

    __table_args__ = (
        Index("ix_locations_geometry", "geometry", postgresql_using="gist"),
        Index("ix_locations_bounding_box", "bounding_box", postgresql_using="gist"),
    )
