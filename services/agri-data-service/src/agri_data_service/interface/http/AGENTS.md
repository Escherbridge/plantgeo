...
concurrency instead of through one query. The other half is `parquet_routes._read_pool`, a
...
fault**: a read that cannot get a slot inside `SLOT_WAIT_SECONDS` is refused as `serving_at_capacity`
rather than queued behind work whose own timeout has not run out.
...
## Every fault leaves as a refusal, and never as a 500
...
copy of the same oversized query against a process already at its ceiling. `_answer` catches, in
order, `ServingRefusalError`, `RequestError`, `TimeoutError`, `duckdb.Error` → `read_over_budget`,
and finally `Exception` → `serving_fault`, logged with a traceback. The last two answer **409**,
which the client treats as permanent: the read is refused, not retried. The bodies never carry the
fault's own message — `_attributed_row`, the listing budget and `render_scalar` all raise text about
internal state, and a refusal states what the caller can do instead.
...
- `asyncio.timeout` around the pool returns the request; it does not stop a DuckDB query that has
  already STARTED, which runs to completion or to its memory ceiling in the worker thread. What
  bounds the damage is the pool: an abandoned read still occupies one of its threads, so a read
  admitted after it queues rather than opening a session beside it, and the process ceiling holds.
  A read still QUEUED when its timeout fires is cancelled and never runs at all.
- A read that times out releases its admission slot while its query is still running, so the next
  caller is admitted and then waits on the pool. It is answered `read_timed_out` rather than
  `serving_at_capacity`; both are refusals, but the code names the wrong one.
