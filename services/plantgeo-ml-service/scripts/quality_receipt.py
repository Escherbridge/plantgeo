...
DIGEST_DOMAIN: Final = b"plantgeo.plantgeo-ml-service.quality-receipt.v2"
...
#: extended mypy scope covers, the lock files fix the tool and library versions, and
#: `mypy.ini`/`ruff.toml` define what "mypy pass" and "lint pass" mean at all. There is no
#: migration machinery here: this service is zero-Postgres (decision D5).
DIGEST_DIRECTORIES: Final[tuple[str, ...]] = ("src", "tests", "scripts")
DIGEST_FILES: Final[tuple[str, ...]] = ("pyproject.toml", "uv.lock", "mypy.ini", "ruff.toml")
