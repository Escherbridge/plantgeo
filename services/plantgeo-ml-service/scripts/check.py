"""Run the service's validation checks, and optionally lock the swept tree into a quality receipt.

Ported from agri-data-service's `scripts/check.py`. The batch and changed-path test selection is
deliberately not carried over: it is keyed on that service's lane directories, and a selection map
describing directories this service does not have would be a rule nobody could trust. The receipt
mechanism and the committed-tree guard are unchanged. See `scripts/AGENTS.md`.
"""
...
if TYPE_CHECKING:
    ...
    #: Reads git's staged view of the digest inputs. Injected so a test needs no repository.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Run the plantgeo-ml-service validation checks.")
...
def print_report(results: Sequence[CheckResult]) -> None:
    """Print a consolidated validation report naming every failure, not only the first."""
...
def _staged_blob_ids(service_root: Path) -> dict[str, str]:
    ...
    record per path; a conflicted path instead carries stages 1, 2 and 3 and no stage-0 record at
    all, and keying by path alone would let whichever record printed last stand in for "the" staged
    content, certifying one arbitrary side of the conflict as this tree's bytes.
    """
    listing = _git_output(("ls-files", "-s", "-z", *_digest_pathspec()), service_root)
    staged: dict[str, str] = {}
...
def main(
    ...
    *,
    ...
    """Run the validation checks and return a consolidated status."""
    parser = build_parser()
    arguments = parser.parse_args(argv)
    if arguments.list:
        if arguments.only or arguments.write_receipt:
            parser.error("--list cannot be combined with run selection or receipt options")
        for check in CHECKS:
            print(check.name)
        return 0
