...
from plantgeo_ml_service.planes.artifact_reads import KNOWN_ARTIFACT_KINDS
from plantgeo_ml_service.planes.routes import read_artifacts
from plantgeo_ml_service.planes.wire import ARTIFACT_ABSENT_NO_ARTIFACT, CLAIM_TIER, OUTCOME_CONTENT, OUTCOME_REFUSED
...
async def test_a_model_kind_this_service_does_not_write_is_rejected_by_name(
    ...
    assert body["outcome"] == OUTCOME_REFUSED
...
async def test_the_retired_underscore_spelling_is_refused_by_naming_the_two_accepted_kinds(
    artifact_store: ServingHarness,
) -> None:
    """The vocabulary is hyphenated; a client on the old spelling is told the accepted kinds."""
    assert not artifact_store.objects

    body = body_of(await read_artifacts(NO_REQUEST, "analog_ensemble"))

    assert body["error"]["code"] == refusals.ARTIFACT_KIND_UNKNOWN
    assert "analog-ensemble" in body["error"]["message"]


def test_every_accepted_artifact_kind_is_named_verbatim_in_the_route_docstring() -> None:
    """A client reads the docstring; a vocabulary it does not list is a kind someone has to guess."""
    documented = read_artifacts.__doc__ or ""

    assert all(f"`{kind}`" in documented for kind in KNOWN_ARTIFACT_KINDS)
    assert KNOWN_ARTIFACT_KINDS == ("analog-ensemble", "fire-risk")
