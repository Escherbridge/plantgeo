...
from plantgeo_ml_service.method.ml.covariate_wind_model import (
    CovariateMatrix,
...
def test_the_feature_checksum_is_the_foundation_digest_of_the_foundation_rendering() -> None:
    """This module used to carry its OWN canonical_json with `ensure_ascii` left at its default.

    Two renderings of the same document meant two digests, so a receipt written here and a receipt
    written by anything using `foundation.canonical` disagreed the moment a name was not ASCII. The
    assertion is written against the foundation helpers directly, not against a stored constant, so
    it fails if either side of that equality is ever forked again.
    """
    feature_names = ("wind_speed_lag_1", "température_moyenne_roll_mean_7")
    document = {
    ...
    checksum = feature_code_checksum(feature_names, schema_version="agri_covariates_v1")

    assert checksum == sha256_digest(canonical_json(document))
    assert "température" in canonical_json(document)  # ensure_ascii=False, not a unicode escape
