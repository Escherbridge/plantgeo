"""Evaluation-only rolling-origin ridge forecaster over the covariate layer.

Layer L1 (method/ml): pure math over an already-loaded covariate matrix. Rationale, the scope
limits, the leakage argument, the sample-size caveat and the `as_of_mode: global` deviation live in
`AGENTS.md` (this directory). Loading the matrix and persisting a receipt belong to `pipeline/`.
"""
...
# SCHEMA_VERSION, TARGET_SIGNAL_NAME and DEFAULT_ORIGIN_COUNT have no caller inside this module:
# phase 2A's Parquet covariate reader binds them when it loads the matrix. See `AGENTS.md`.
