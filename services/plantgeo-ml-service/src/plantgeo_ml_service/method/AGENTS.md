...
- **May NOT import**: `warehouse`, `pipeline`, `planes`, `interface`, `polars`, `pyarrow`, `duckdb`,
  `boto3`, `sanic`, `sqlalchemy`, `httpx`, `asyncpg`, `click`.

## Sub-packages
- `method/monte_carlo/`: Seasonal-anomaly daily NDVI bootstrap simulations.
- `method/ml/`: Species fit ranker, expert label plane, Analog Ensemble k-NN forecaster, split-conformal
  calibration, covariates v2, the ridge wind model, seasonal candidates/evaluation/lineage.
- `method/kernels/`: Python reference kernels plus the Mojo-backed dispatch (phase 3); may import
  `foundation` only.
...
they need lives one layer up in `pipeline/`, because this layer may not
import a storage client -- `tests/test_layer_import_contract.py` enforces that, and the
...
- **`ENVELOPE_TERM_SUPPORT` is the load-bearing table.** `direct` terms are derived
  ...
  means editing this table **and** the expert-label envelope validator that reads the
  exported Parquet label plane in the same review, or the two disagree.
