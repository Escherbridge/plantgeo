...
```bash
uv run agri-cli parquet-drain --dry-run --selection missing --layer sensors   # confirm the count first
uv run agri-cli parquet-drain --selection missing --layer sensors --progress
```
