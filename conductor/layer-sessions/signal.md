...
```bash
uv run agri-cli parquet-drain --dry-run --selection missing --layer signal   # confirm the count first
uv run agri-cli parquet-drain --selection missing --layer signal --progress
```
