# Session brief -- `vegetation` end to end
...
You own the **`vegetation`** lane and nothing else. Twelve sibling briefs exist and other sessions may
...
|---|---|
| nature | `daily_series` |
| history floor | `2022-08-05` |
| cadence | 1 day(s) |
| publication lag | 7 day(s) |
| forecastable | YES |
...
> Governed forecastable plane holds 2022-08-05..2026-08-04, the deepest record of any lane. Lag 7 is the MEASURED median gap between observation days -- worse than Sentinel-2's nominal 5-day revisit because cloud screening removes scenes. Most days in the window are correctly a governed absence.
...
| measure | count |
|---|---|
| base rung days (z13, has data) | **1195** |
| governed-absence days (z13) | **280** |
| days with a COMPLETE ladder | **990** |
| days with an INCOMPLETE ladder | **205** |
...
### 1. The 205 incomplete-ladder days -- BLOCKED, and not by effort

`vegetation` is one of exactly three lanes whose coarse rungs CANNOT be derived. Measured
2026-08-25 by running the repair against production:

```
TierDerivationError: vegetation: the tier derivation names coordinate column(s)
[cell_longitude, cell_latitude] that the base table does not carry
```

The base rungs for these 205 days were written before the coordinate columns existed.
...
retraction of the affected base days, then `parquet-drain --selection missing --layer vegetation`,
which IS source-connected -- so read the collision rule above before you start it.

**Confirm the boundary before retracting anything.** 990 of this lane's 1195
...
```bash
uv run agri-service data parquet-drain --dry-run --selection missing --layer vegetation   # confirm the count first
uv run agri-service data parquet-drain --selection missing --layer vegetation --progress
```

This is SOURCE-CONNECTED. It queries Postgres, so the collision rule applies.

### 3. 280 governed-absence days -- DO NOT "FIX" THESE

These are days upstream legitimately cannot serve, correctly recorded at z13. They are **not**
missing data and they need **no** export. Governed forecastable plane holds 2022-08-05.
...
- [ ] Every day in the declared window is `data` or a governed `absent` at **all four rungs** --
      not just z13.
- [ ] `--dry-run --selection ladder` reports `incomplete_ladder_days: 0` for `vegetation`. **Expect it
      ...
      alarm.
- [ ] `--dry-run --selection missing` reports `missing_days: 0` for `vegetation`.
...
1. The before and after of both censuses for `vegetation`, as the raw numbers.
