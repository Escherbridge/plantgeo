# Session brief -- `weather-observations` end to end
...
You own the **`weather-observations`** lane and nothing else. Twelve sibling briefs exist and other sessions may
...
> FALLBACK -- NOT DECLARED ANYWHERE, AND THE GUESS IS DELIBERATELY SHALLOW. docs/lanes/weather-observations.md describes the NASA POWER / ERA5-Land archive, which is the SIGNAL stream, not this lane. The producer THIS lane exports (ingest/open_meteo.py's WEATHER_LAYER current-conditions poll into geo.features) has NO contract content at all: no declared cadence, horizon, historical depth or known-gaps list. 2026-08-01 is a conservative recent floor chosen so a wrong guess costs a few dozen phantom gap-days instead of thousands.
...
| measure | count |
|---|---|
| base rung days (z13, has data) | **20** |
| governed-absence days (z13) | **2** |
| days with a COMPLETE ladder | **20** |
...
- [ ] Every day in the declared window is `data` or a governed `absent` at **all four rungs** --
      not just z13.
- [ ] `--dry-run --selection ladder` reports `incomplete_ladder_days: 0` for `weather-observations`. **Expect it
      ...
      alarm.
- [ ] `--dry-run --selection missing` reports `missing_days: 0` for `weather-observations`.
...
1. The before and after of both censuses for `weather-observations`, as the raw numbers.
