# Session brief -- `drought` end to end
...
You own the **`drought`** lane and nothing else. Twelve sibling briefs exist and other sessions may
...
| history floor | `2022-08-09` |
| cadence | 7 day(s) |
| publication lag | 4 day(s) |
| forecastable | no |
...
> A USDM map is a dated publication; valid_date IS the release's own fact rather than a day anyone observed. MEASURED against production: min=2022-08-09, max=2026-08-18, 209 distinct releases, 1,045 rows. The ingest code's USDM_ARCHIVE_START of 2000-01-04 is an ARCHIVE CAPABILITY, not what production holds -- using it would invent ~1,100 phantom weeks. cadence 7 because valid_date is always a Tuesday and the floor is a Tuesday, so the step lands on real release days.
...
| measure | count |
|---|---|
| base rung days (z13, has data) | **209** |
| governed-absence days (z13) | **2** |
| days with a COMPLETE ladder | **209** |
...
All 209 base days carry a complete four-rung ladder, nothing is missing, and nothing is
...
- [ ] Every day in the declared window is `data` or a governed `absent` at **all four rungs** --
      not just z13.
- [ ] `--dry-run --selection ladder` reports `incomplete_ladder_days: 0` for `drought`. **Expect it
      ...
      alarm.
- [ ] `--dry-run --selection missing` reports `missing_days: 0` for `drought`.
...
1. The before and after of both censuses for `drought`, as the raw numbers.
