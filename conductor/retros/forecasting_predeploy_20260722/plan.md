...
- [x] Rehearse the verified custom-format restore and PostgreSQL 18 extension parity before any Railway migration.
  *Done 2026-08-02/03, documented in `services/agri-data-service/plans/postgresql-18-migration-rehearsal-2026-08-02.md`
  §9–11: 928 MB custom-format restore into fresh PG18, extension parity vs pg16 baseline
  (postgis 3.6.4, timescaledb 2.29.0, vector 0.8.5, pgcrypto 1.4), migrations 0001→0016 clean,
  297/302 contract tests, two real blockers found and cleared. No Railway resource touched.*
...
- [x] Do not deploy or mutate Railway from this track. *Honored (every evidence file
  records "No Railway resource was touched"); deployment later shipped through the
  separate reviewed pipeline (commit `778a271`, migration-decision-packet-2026-08-08).*
- [x] Do not schedule materialized-view refresh or forecast execution yet. *Honored and
  still in force for the forecasting MV — `infra/railway/README.md` §8 still forbids a
  forecast-refresh cron; verified no scheduler code targets it as of 2026-08-14.*
- [x] Do not promote strategy recommendations or populate `strategy_selection`. *Honored;
  plane still gated and empty per `infra/railway/README.md` and `models/AGENTS.md`.*
- [x] Do not delete the named warehouse volume, raw receipt cache, historical Parquet lake, or older backups while pruning task context. *Honored; backups verified present on disk
  2026-08-14 (`C:\tmp\plantgeo-warehouse-backups\`, pre-0006 dump byte-count matches its
