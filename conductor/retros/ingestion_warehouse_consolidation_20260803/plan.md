...
- [x] Author `20260803_0018_*` — drop `finalize_*`, `verify_*`, `enforce_*`,
...
- [x] Keep all nine ML-serving tables, every checksum column and its format CHECK,
...
- [x] **Owner applies to production.** Agents are not permitted to; `.env` in that directory
      points at production, so every command needs an explicit `$env:DATABASE_URL_SYNC` override.
      *Verified applied 2026-08-14: prod `alembic_version = 20260808_0019` (past 0018);
      enforcement objects absent, nine ML-serving tables and 27 checksum CHECKs present.*
...
- [x] Community features do not work on the map. *Closed 2026-08-14: full
      pipeline live (interventions tRPC → /moderation → `geo.intervention_tiles()`
      → registered map layers); see lane-H-browser-qa.md and
      community_intervention_lifecycle_20260814.*
- [x] Navigation can trap the user in the organization settings page. *Closed
      2026-08-14: fixed in 0e194f8/2869fb5 — `<main>` is the scroll surface,
      header pinned with the Dashboard exit link; see lane-H-browser-qa.md §4.1.*
