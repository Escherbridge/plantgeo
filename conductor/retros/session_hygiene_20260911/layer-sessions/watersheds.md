# Session brief -- `watersheds` end to end
...
You own the **`watersheds`** lane and nothing else. Twelve sibling briefs exist and other sessions may
...
> Exactly ONE load day exists, 2026-08-07, all 9,396 rows, and the boundaries are a snapshot rather than a series. A HUC12 boundary is a reference fact WITH A VERSION, so the partition day comes from sql/pipeline/lane_watermark_watersheds.sql -- geo.features' change-gated updated_at/created_at for this layer -- and the floor is INERT.
...
lane comes from its `sql/pipeline/lane_watermark_watersheds.sql`, never from the
...
- [ ] Every day in the declared window is `data` or a governed `absent` at **all four rungs** --
      not just z13.
- [ ] `--dry-run --selection ladder` reports `incomplete_ladder_days: 0` for `watersheds`. **Expect it
      ...
      alarm.
- [ ] `--dry-run --selection missing` reports `missing_days: 0` for `watersheds`.
...
1. The before and after of both censuses for `watersheds`, as the raw numbers.
