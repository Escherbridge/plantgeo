# Session brief -- `signal` end to end
...
You own the **`signal`** lane and nothing else. Twelve sibling briefs exist and other sessions may
...
|---|---|
| nature | `daily_series` |
| history floor | `2022-04-30` |
| cadence | 1 day(s) |
| publication lag | 9 day(s) |
| forecastable | YES |
...
> Whole plane measured 2022-04-30..2026-08-06 across BOTH producers. Lag 9 is ERA5-Land's PUBLICATION_LAG_DAYS; NASA POWER's is 5. The LARGER is deliberate -- at lag 5 the four newest days would be declared missing while ERA5-Land has genuinely not published them.
...
| measure | count |
|---|---|
| base rung days (z13, has data) | **1560** |
| governed-absence days (z13) | **10** |
| days with a COMPLETE ladder | **1338** |
| days with an INCOMPLETE ladder | **222** |
...
### 1. The 222 incomplete-ladder days -- BLOCKED, and not by effort

`signal` is one of exactly three lanes whose coarse rungs CANNOT be derived. Measured
...
The base rungs for these 222 days were written before the coordinate columns existed.
...
retraction of the affected base days, then `parquet-drain --selection missing --layer signal`,
which IS source-connected -- so read the collision rule above before you start it.

**Confirm the boundary before retracting anything.** 1338 of this lane's 1560
...
```bash
uv run agri-service data parquet-drain --dry-run --selection missing --layer signal   # confirm the count first
uv run agri-service data parquet-drain --selection missing --layer signal --progress
```
...
- [ ] Every day in the declared window is `data` or a governed `absent` at **all four rungs** --
      not just z13.
- [ ] `--dry-run --selection ladder` reports `incomplete_ladder_days: 0` for `signal`. **Expect it
      ...
      alarm.
- [ ] `--dry-run --selection missing` reports `missing_days: 0` for `signal`.
...
1. The before and after of both censuses for `signal`, as the raw numbers.
