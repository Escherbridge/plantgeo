# Session brief -- `soil-survey` end to end
...
You own the **`soil-survey`** lane and nothing else. Twelve sibling briefs exist and other sessions may
...
| history floor | `2025-08-26` |
| cadence | 1 day(s) |
| publication lag | 0 day(s) |
| forecastable | no |
...
> Vintage-only, NOT a daily series -- one live vintage per delineation, keyed by survey-area publication. The floor is the measured saverest span start (2025-08-26..2026-03-19) and is INERT: the partition day comes from sql/pipeline/lane_watermark_soil_survey.sql, which takes the newer of SSURGO's own saverest vintage and the day this warehouse's lazily-warmed published set last grew.
...
| measure | count |
|---|---|
| base rung days (z13, has data) | **0** |
| governed-absence days (z13) | **0** |
| days with a COMPLETE ladder | **0** |
...
### 1. 1 unfinished day/version -- parts on disk, no completion marker
...
### 2. Coverage is UNKNOWN, which is not the same claim as current
...
lane comes from its `sql/pipeline/lane_watermark_soil_survey.sql`, never from the
...
- [ ] Every day in the declared window is `data` or a governed `absent` at **all four rungs** --
      not just z13.
- [ ] `--dry-run --selection ladder` reports `incomplete_ladder_days: 0` for `soil-survey`. **Expect it
      ...
      alarm.
- [ ] `--dry-run --selection missing` reports `missing_days: 0` for `soil-survey`.
...
1. The before and after of both censuses for `soil-survey`, as the raw numbers.
