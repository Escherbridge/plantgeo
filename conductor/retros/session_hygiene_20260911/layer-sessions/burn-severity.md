# Session brief -- `burn-severity` end to end
...
You own the **`burn-severity`** lane and nothing else. Twelve sibling briefs exist and other sessions may
...
| history floor | `2020-11-24` |
| cadence | 1 day(s) |
| publication lag | 7 day(s) |
| forecastable | no |

**Forecast:** not applicable to this nature.
...
> MTBS publishes fire-year cohorts quarterly and each release IS a dated fact -- five of them, 2020-11-24..2024-08-22. cadence_days stays 1 DELIBERATELY, unlike drought's 7: those five dates do not sit on any fixed step from the floor, so a cadence above one would step straight past real releases. The ~2,000 honest absence markers this costs are the PRICE of an irregular release series, not a bug.
...
| measure | count |
|---|---|
| base rung days (z13, has data) | **4** |
| governed-absence days (z13) | **2089** |
| days with a COMPLETE ladder | **4** |
...
```bash
uv run agri-service data parquet-drain --dry-run --selection missing --layer burn-severity   # confirm the count first
uv run agri-service data parquet-drain --selection missing --layer burn-severity --progress
```
...
### 3. 2089 governed-absence days -- DO NOT "FIX" THESE

These are days upstream legitimately cannot serve, correctly recorded at z13. They are **not**
missing data and they need **no** export. MTBS publishes fire-year cohorts quarterly and each release IS a dated fact -- five of them, 2020-11-24.
...
- [ ] Every day in the declared window is `data` or a governed `absent` at **all four rungs** --
      not just z13.
- [ ] `--dry-run --selection ladder` reports `incomplete_ladder_days: 0` for `burn-severity`. **Expect it
      ...
      alarm.
- [ ] `--dry-run --selection missing` reports `missing_days: 0` for `burn-severity`.
...
1. The before and after of both censuses for `burn-severity`, as the raw numbers.
