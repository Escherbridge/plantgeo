...
## Phase 3: Mojo kernels — one push (LANDED cc6c6868 2026-09-19; Docker stage opt-in and unproven on Railway; production dispatch stays python)

- [x] Task: `pixi.toml` / `mojoproject.toml` pinning Mojo 1.0.x; `kernels/` Mojo sources:
      ...
      photoperiod).
- [x] Task: `method/kernels/__init__.py` dispatch: `PLANTGEO_ML_KERNELS=python` uses the pure
...
- [x] Task: `tests/kernels/test_parity_*.py` — golden outputs generated by the Python reference,
      asserted equal (exact for KNN and the bootstrap; features `rtol=1e-12`, amended 2026-09-19: one ulp of 43,200 s of daylight is 7.3e-12, so an absolute 1e-12 was below the representable precision) under both dispatch modes;
      skipped with a named reason when Mojo is absent, never silently passing.
- [x] Task: `Dockerfile` Mojo build stage (pixi install, `mojo build --emit shared-lib`), copied into
...
- [x] Task: benchmark receipt (`benchmark` stdlib vs scikit-learn brute force on the 1,568-cell
