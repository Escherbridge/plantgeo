...
| --- | --- | --- |
| A — data steward | [Source register](./source-register.md), public-source discovery, lightweight provenance notes, and data-quality acceptance checklist | Public non-synthetic Kaggle datasets are admissible after local schema/checksum profiling; utility data needs documented authorization/consent |
...
- The isolated local stack starts from documented configuration, applies a
  ...
  abstention states.
- Every candidate source has a source-register decision plus a source/terms
  note; every accepted artifact release has a payload checksum and basic
  provenance. Utility data additionally has documented authorization and an
  approved AOI boundary. Explicitly synthetic or unsupported sources remain
  out of the import plane.
