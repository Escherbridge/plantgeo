...
# PlantGeo — Runbook archive, sessions through 2026-08-29

This is the historical tail of `RUNBOOK.md`, split out on 2026-09-04 when the live file reached 9,771
lines. **Nothing here was edited; it is the original text, verbatim, from the first `## HISTORICAL`
heading onward.** It is retained as evidence, not as instruction.

## How to read it safely

Sections are ordered newest-first, the same as they were in the live file, so a lower section is older
and more likely to be wrong. Several of these sections were superseded *by later sections inside this
same archive*. The headers that used to warn about that were rewritten into the live file's summary, so
when two sections here disagree, **the later one wins and the live RUNBOOK wins over both**.

The three reversals worth knowing before you trust anything below:

- **The partition swap was refuted by measurement.** A controlled probe found byte-identical plan text
  with `enable_partitionwise_aggregate`/`_join` off and on, because each census matview joins the
  never-co-partitionable 11-row `geo.layers` before aggregating. Partitioning `geo.features` delivers
  zero relief to the workload that was blowing the memory cap. Sections proposing it are dead.
- **`EXPLAIN` cost is meaningless for these tile functions** — it prices a 0-row layer identically to a
  186,904-row one, which invalidates every cost-based conclusion in the older sections.
- **Every `pg_stat_*` counter here was measured over an unknown window.** `stats_reset` is NULL on
  production, which does not mean "since forever": a crash discards cumulative stats and leaves it NULL
  again. Read `idx_scan = 0` as "zero over an unbounded-below window", never as "never read".

Anything operational — what runs, what is authorized, what is owed — lives in the live `RUNBOOK.md` and
the track files it points at. Do not act from this file.
