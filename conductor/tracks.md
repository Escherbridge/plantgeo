...
| [parquet_duckdb_pivot_20260823](tracks/parquet_duckdb_pivot_20260823/) | active | feature | **The pivot itself, chartered retroactively 2026-08-23** -- waves 1-3 shipped with no track, so the largest workstream in the repo was tracked only in RUNBOOK prose. Shipped: twelve lanes (`abf777f`), lane natures + watermark-driven static lanes + the conformed calendar (`b794e98`), the per-kind schema lookup that unblocked five forecasters (`269d299`), and three of four adversarial-review defects (`ef789f7`). Live in prod: 1,240 objects across twelve lanes. **Wave 4, in order:** the sub-day version fix for evacuation-zones (LIFE-SAFETY -- the lane can serve a stale evacuation level for up to a day), then paying down what the latch fix created (auto-retraction, the `_static_lane_census` test gap, matview re-publish visibility, two doc surfaces), then the fire-risk feature plane, then wiring every Open-Meteo product. Carries prebaked partitions at `ef789f7`; s1 and s2 both touch `gap_fill.py` and must not run concurrently. See RUNBOOK 0.29. |
