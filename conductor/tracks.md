...
| [parquet_duckdb_pivot_20260823](tracks/parquet_duckdb_pivot_20260823/) | active | feature | **The pivot itself.** Shipped: twelve lanes (`abf777f`), lane natures + watermark static lanes (`b794e98`), per-kind schema (`269d299`), review defects (`ef789f7`), the sub-day version fix (`ceeb2c9`) and the orphan-part blocker its review found (`3b7ecfb`), and **the zoom axis end to end plus soil-survey's streaming export (`68da7af`)** -- `zoom=NN` above `year=`, uniform z0/z5/z9/z13 ladder, and `MAX_SOIL_SURVEY_POLYGON_KEYS` DELETED, which unblocked a lane that had never written a single object. Gate 3,936 passed / 3 skipped. **Remaining, in dependency order (RUNBOOK 0.33.3):** A. tier derivation z9/z5/z0 from base z13 in Polars/DuckDB -- **blocks everything visual, the map is empty above z13 until it lands**; B. bulk Postgres drain (13,037 lane-days, 69% `fire-detections`) then stop the cron -- build, run, THEN stop; C. `interface/http` serving API, whose coverage endpoint must **reproduce** `getSliderCapabilities`' contract so the slider needs zero changes; D. repoint the six ready tRPC procedures; E. upstream API → Parquet per lane; F. new lanes incl. `soil-field`, which has none; G. static lookups leave the lane registry. **Two open decisions and two deferred hazards in RUNBOOK 0.33.2/0.33.4** -- chiefly that a half-written release can read as `current`. Partitions re-baked at `68da7af` as d1-d5. See RUNBOOK 0.33, then 0.32. |
| [postgres_shrink_ingest_repoint_20260825](tracks/postgres_shrink_ingest_repoint_20260825/) | planned | feature | **Successor to the pivot's item E** (RUNBOOK 0.33.3 "forward path per lane"), which moves here. Bridge-then-cut: P0 restores the ingest `cronSchedule` as the transition writer (deadline ~2026-08-31, NWS keeps ~6 days); P1 collapses alembic to a greenfield baseline; P2-P4 build direct-from-source Parquet writers lane by lane with a parity harness (rolling-window lanes first, governance lanes last); P5 retires each lane's Postgres path after parity; P6 drops `signal_observation` and the data-plane rows. Blocked on pivot slices d1 (coarse rungs + signal z13 re-export), d3 (serving API) and d4 (tRPC repoint), never re-owning their files. Decisions and evidence: RUNBOOK 0.40. |
| [soil_survey_lane_shape_20260825](tracks/soil_survey_lane_shape_20260825/) | pending | design-decision | **The only Parquet lane with zero base_days.** Measured 2026-08-25: 238,986 rows exist but export raises on 200,001-key dataset vs 200,000 cap, draining nothing. RUNBOOK §0.29.1 (wrong) vs §0.31.1 (corrected). **Owner framing: static datasets may need different grain than rolling time series.** Four open design questions: sealed single partition vs rolling days? Memory/time cost of 200K+ key export? Zoom ladder semantics for static data? Serving API contract? Deliberately deferred from 2026-08-25 session by owner decision. Blocked on owner decision on lane shape. |
...
| [fire_feature_plane_validation_20260824](tracks/fire_feature_plane_validation_20260824/) | blocked | feature | Converts AUC 0.725 from an **in-sample association** into a defensible skill estimate, plus calibration to a probability (without which "carbon at risk = stock × probability" has no second term). **Blocked on the 23-year `fire-detections` hole (2003–2025)** — which is **not new work**: it is `parquet_duckdb_pivot_20260823` **item B**, where fire-detections is 69% of the 13,037 lane-days. Traps recorded: uneven per-year coverage (2000 has 35 days, 2003 has 1), the silent upstream record cap, unmodelled ignition, and the **MODIS→VIIRS sensor change**, which is the likeliest way to manufacture a fake trend. |
