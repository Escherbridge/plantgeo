...
| [parquet_duckdb_pivot_20260823](tracks/parquet_duckdb_pivot_20260823/) | active | feature | **The pivot itself, chartered retroactively 2026-08-23.** Shipped: twelve lanes (`abf777f`), lane natures + watermark-driven static lanes + conformed calendar (`b794e98`), per-kind schema lookup (`269d299`), three of four review defects (`ef789f7`), the sub-day version fix (`ceeb2c9`) and the orphan-part blocker its review found (`3b7ecfb`). Live in prod: 2,274 objects; four series lanes are 100% backfilled and 13,037 lane-days remain, 69% of it `fire-detections` alone. **Wave 4 re-scoped by four owner decisions 2026-08-23 (RUNBOOK 0.32):** Postgres becomes a one-time CUT-OFF, not a source -- bulk-drain it to Parquet, then the upstream APIs write Parquet directly and each lane deprecates its Postgres source. A **`zoom=` partition key** goes above `year=` with a uniform z0/z5/z9/z13 ladder, tiers derived in Polars/DuckDB from the base Parquet; `MAX_SOIL_SURVEY_POLYGON_KEYS` is DELETED in favour of chunked streaming (full granularity, no cap). Then the `interface/http` serving API and the Next.js repoint. **0.30.3's layout freeze is deliberately retired for the zoom axis**; the existing 2,274 objects are DISCARDED, not migrated. Partitions re-baked at `3b7ecfb`: **z0 (`paths.py`) is the foundation and MUST run alone** -- it breaks every call site at type-check time on purpose, and its broken-call-site report is the work plan for z1-z5. Also newly in scope: `soil-field` has NO lane at all. See RUNBOOK 0.32, then 0.31. |
