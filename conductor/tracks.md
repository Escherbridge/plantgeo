...
| [parquet_duckdb_pivot_20260823](tracks/parquet_duckdb_pivot_20260823/) | active | feature | Historical data construction is complete on integrated `main` `949e20e`: the immutable 46,146,568-row canonical snapshot and governed climate/soil product lanes reconcile exactly. `d1` and `d5` are superseded-complete. `d3` routes/client are code-complete but production-blocked by the API `/ready` timeout; `d4` tRPC/capability repoint and browser acceptance remain pending. PostgreSQL stays intact until the successor retirement gates clear. |
| [postgres_shrink_ingest_repoint_20260825](tracks/postgres_shrink_ingest_repoint_20260825/) | active | feature | **Successor forward-path and retirement track.** Cron configuration and migration contracts through `20260827_0027` are integrated, but production cadence and live migration head still need verification. Direct/queued forward paths exist for fire detections, water gauges, vegetation, and weather audit; snapshot-only climate/soil products still need durable forward ownership. Also pending: shared `parquet_ops/`, the CLI split/rename, and the agent/MCP repoint. P5/P6 have not started. Next: repair `/ready`, probe routes, finish repoints, run browser/environment gates, prove forward parity, then retire PostgreSQL per lane. |
