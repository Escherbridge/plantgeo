...
| Herbaria specimens | [PNW Herbaria admission](tracks/pnw_herbaria_source_admission_20260911/plan.md) | Coordinate-withholding policy is unproven for WTU/UBC and the public-metadata search is exhausted (neither EML, IPT page, nor Canadensys's own about page states one); resolution needs an institutional reply or an explicit owner risk decision, both narrowed to concrete next steps 2026-09-13. Custody/archive-control preflight narrowed to five named operator decisions against already-implemented controls. Occurrence Parquet lane, species-profile Parquet lookup port, and map-experience implementation merged to main 2026-09-13 (PR #6); routes/agent tools remain deliberately unmounted pending the tracks' own independent-review gate. No archive acquisition, no occurrence release admitted. |
