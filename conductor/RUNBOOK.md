...
| Herbaria specimens | [PNW Herbaria admission](tracks/pnw_herbaria_source_admission_20260911/plan.md) | Owner risk decision 2026-09-13 accepted per-record `informationWithheld`/`dataGeneralizations` as sufficient coordinate-policy evidence and adopted the custody proposal as-is, clearing pre-acquisition gates for both collections. UBC v16.43 acquired into quarantine and passed archive-safety inspection (outcome `release_accepted`); WTU acquisition deferred behind it per the one-transfer-at-a-time budget. Remaining: field-map reconciliation and a two-release (v16.42/v16.43) native-ID comparison before any occurrence release is admitted; `admitted_releases` still empty. Occurrence Parquet lane, species-profile Parquet lookup port, and map-experience implementation merged to main 2026-09-13 (PR #6); routes/agent tools remain deliberately unmounted pending the tracks' own independent-review gate. |
