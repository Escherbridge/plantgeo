...
`CRON_SECRET` is retired: every ingestion cron container — `plantgeo-ingest-cron` (`agri-service data ingest-all`) and, once created, each of the eight per-layer services in `docs/deployment.md`'s
