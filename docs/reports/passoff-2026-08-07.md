...
./run-backfill.sh ops jobs-status                                  # is it healthy
./run-backfill.sh ops jobs-plan-lane      --lane firms-archive     # idempotent
./run-backfill.sh ops jobs-reconcile-lane --lane firms-archive     # dry run; --apply to settle
./run-backfill.sh ops jobs-run            --lane firms-archive     # one bounded slice
./run-backfill.sh ops validate-streams --bbox "-125,42,-111,49" --format markdown --output ../../docs/reports/x.md
```
