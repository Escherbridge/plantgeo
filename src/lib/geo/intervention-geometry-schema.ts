/**
 * Intervention geometry schema — client-safe (no Node.js/server dependencies).
 * Shared by the server validator (`@/lib/server/services/intervention-geometry`,
 * which re-exports these) and browser components that need the `InterventionGeometry`
 * type without pulling in server-only code.
 */
import { z } from "zod";
...
/** Point/Polygon/MultiPolygon geometry an intervention site may carry. */
