...
Both `sensorsLayer` and `evacuationZonesLayer`/`evacuationZonesOutlineLayer` (`src/lib/map/layers.ts`) are registered exactly like every other style-backed toggle -- `LAYER_REGISTRY` entries with `renderKind: "style"`, no hand-edits needed in `LayerManager`, `Legend`, or `STYLE_LAYER_TOGGLE_MAP`, all of which derive from the registry. They stay style-backed after the Parquet cutover, deliberately: `LayerManager`'s visibility, opacity and date-filter appliers all walk `LAYER_REGISTRY.styleLayerIds`, and emptying it would disarm all three at once. `evacuation-zones` paints on `severity`, populated unconditionally alongside `evacuation_level_label` whenever the Oregon OEM feed reports an `evacuationLevel` -- see `evacuation_zones.py build_evacuation_zone_write`.

**The SQL fix this section used to be waiting on is moot, and the outcome is better than the fix.** `sensors` paints on `network`, the one property `sensors.py._matches_networks` guarantees is set on every row the NWS producer writes (it rejects a station before collection otherwise). `geo.sensor_tiles()` never carried it -- the function emitted `sensor_type`/`status`/`name`, none of which any producer populates, so every one of the ~750 published stations drew in the neutral grey fallback, and `drizzle/0010_sensor_tile_properties.sql`'s repair sat unapplied. Wave C of `environmental_postgres_retirement_20260904` moved the layer to `environmental.getSensorStations`, and `presentParquetSensorStations` emits `network` off the lane's own column. The grey fallback now means what it says: a station whose network the producer really did not report.

`LayerManager` owns a **fourth applier** for these two and for `burn-severity`/`watersheds`: `applyParquetFeatureData`, which writes each read's collection onto the empty GeoJSON source `styles.ts` declares. It runs on `style.load` as well as on new data, because a basemap swap rebuilds every source from that empty spec -- the same reason visibility, filter and opacity are all re-applied there. It is deliberately NOT in the `styledata` frame with the other two: those rebuild an expression per layer, while this re-serializes up to 9,396 basins, and `styledata` fires as every tile lands.
