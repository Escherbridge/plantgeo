...
/**
 * The active organization's workspace, as the Teams section of the map dock. It governs no
 * layer, so it is one of the three sections that sit below the layer groups.
 *
 * Mounted only while that section is expanded, which is what the dashboard query's `enabled`
 * flag now reads in place of the sheet's `open` prop -- `!!teamId` is all that is left of it,
 * and it still matters: a reader with no organization has nothing to fetch.
 */
export function TeamDetails({ teamId, mapRef }: TeamDetailsProps) {
  ...
  return (
    <div className="flex flex-col">
      {/* The organization's own name, which the dock's static disclosure label cannot
          carry: the label has to read the same before the query resolves as after it. */}
      {dashboard && (
